...
EVENTS: The following events are registered by this plugin and are used to
    ...
    1. ALLIANCE_CHAT_RECV
        ...
        formatted message in the game's chat window.

    2. ALLIANCE_CHAT_SEND

        The AllianceChat plugin will generate this event when the user sends a message via
        the prompt or other ui.

MESSAGES: The table structure that will be consumed and sent by AllianceChat uses a
...

{
    ...
    msg       = "hello chat friends!",
        ...
        into the games chat window. The <Ally> portion is referred to as the "prompt" and
        it's appearance can be changed in settings.lua or via function calls in your own plugin (details elsewhere)
            ...
            3. "factionColor" will be a valid hex color code and will be used to set the display color of the name and guild
                defaults to the color of teh current chars faction using rgbtohex(FactionColor_RGB[tonumber(GetPlayerFaction(id))]) or '127FFFFFF'
